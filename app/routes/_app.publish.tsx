...
function autoGenerateCommitMessage(summary: ChangeSummary): string {
  const parts: string[] = [];
  ...
  if (newStories > 0) parts.push(`add ${newStories} ${newStories === 1 ? "story" : "stories"}`);
  if (deletedStories > 0) parts.push(`remove ${deletedStories} ${deletedStories === 1 ? "story" : "stories"}`);
  if (newObjects > 0) parts.push(`add ${newObjects} ${newObjects === 1 ? "object" : "objects"}`);
  if (deletedObjects > 0) parts.push(`remove ${deletedObjects} ${deletedObjects === 1 ? "object" : "objects"}`);
  if (summary.settings.changed.length > 0) parts.push("update site settings");
  if (summary.landing.changed) parts.push("update homepage");
...
function autoGenerateCommitBody(summary: ChangeSummary): string {
  const lines: string[] = [];

  for (const s of summary.stories.new) {
    lines.push(`+ Story: ${s.title ?? s.story_id}`);
  }
  for (const s of summary.stories.deleted) {
    lines.push(`- Story: ${s.title ?? s.story_id}`);
  }
  for (const o of summary.objects.new) {
    lines.push(`+ Object: ${o.title ?? o.object_id}`);
  }
  ...
  lines.push("");
  lines.push("Published via Telar Compositor");
  return lines.join("\n");
}
...
export default function PublishPage({ loaderData }: Route.ComponentProps) {
  ...
  const autoGeneratedHeadline = autoGenerateCommitMessage(changeSummary);
  const autoGeneratedBody = autoGenerateCommitBody(changeSummary);

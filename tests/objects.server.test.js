 ...
 */
function simulateUpdateObjectAltText(formDataValues) {
    const setCalls = [];
    const mockDb = {
        update: vi.fn(() => mockDb),
        set: vi.fn((values) => {
    ...
    // Mirror the production code: formData.get("alt_text")?.trim() || null
    const raw = formDataValues["alt_text"];
    const alt_text = raw?.trim() || null;

    // Simulate the .set({...}) call with alt_text included
